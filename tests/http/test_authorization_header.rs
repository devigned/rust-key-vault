...
test!(test_basic_auth_parse {
  let headers = Headers::from_raw(&mut mem("WWW-Authorization: Bearer authorization=\"https://login.windows.net/123\", resource=\"https://vault.azure.net\"\r\n\r\n")).unwrap();
  let auth = headers.get::<WwwAuthorization<Bearer>>().unwrap();
